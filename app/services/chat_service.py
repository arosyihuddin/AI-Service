...
class ChatService:
        ...
        try:
            ...
            if material_ids is not None and len(material_ids) > 0:
                ...
                req = DocumentSearchRequest(
                    material_ids=material_ids, query=user_input, k=settings.k_chat
                )
    ...
    async def get_chat_history(self, db: AsyncSession, session_id: str) -> List[Dict]:
        ...
        messages_result = await db.execute(
            select(ChatMessage)
            .filter(ChatMessage.session_id == session_id, ChatMessage.role != "system")
    ...
    async def get_session_history(self, db: AsyncSession, user_id: int) -> List[str]:
        result = await db.execute(
            select(ChatSession)
            .where(ChatSession.user_id == user_id, ChatSession.deleted_at == None)
    ...
    async def _validate_session(self, db: AsyncSession, session_id: str) -> bool:
        result = await db.execute(
            select(ChatSession).filter_by(id=session_id, deleted_at=None)
        )
    ...
    async def _get_messages(self, db: AsyncSession, session_id: str) -> List[Dict]:
        result = await db.execute(
            select(ChatMessage)
            .where(ChatMessage.session_id == session_id, ChatMessage.deleted_at == None)
            .order_by(ChatMessage.created_at.asc())
        )
    ...
    async def _update_material_ids(
        self, db: AsyncSession, session_id: str, material_ids: list[int]
    ):
        getSession = await db.execute(
            select(ChatSession).filter(ChatSession.id == session_id)
        )
    ...
    async def _trim_history(self, db: AsyncSession, session_id: str):
        ...
        result = await db.execute(
            select(ChatMessage).filter_by(session_id=session_id, deleted_at=None)
        )
    ...
    def _merge_messages_context(self, messages: str, context: str) -> str:
        ...
        instructions = (
            ...
            "2. SETIAP KALIMAT HARUS MEMILIKI CITASI [n]\n"
            "3. JIKA TIDAK ADA DI CONTEXT, JAWAB 'Tidak ditemukan dalam referensi'\n"
            "4. **SELALU GUNAKAN BAHASA INDONESIA**\n"
            "5. URUTKAN REFERENSI SESUAI KEMUNCULAN\n\n"
        )

        # Contoh format wajib
        example = (
            "## Contoh Format:\n"
            "Pertanyaan: Apa tiga jenis machine learning?\n"
            "Jawaban: "
            "Tiga jenis utama machine learning adalah supervised learning [1], "
            "unsupervised learning [2], dan reinforcement learning [3].\n\n"
            "**Referensi:**\n"
            "1. [ML Basics: Modul 2 Hal 5](link1)\n"
            "2. [Data Mining: Modul 3 Hal 12](link2)\n"
            "3. [AI Advanced: Modul 1 Hal 8](link3)\n\n"
        )

        context_str = f"## Context Tersedia:\n{context}"
        return f"Pertanyaan: {messages}\nContext: {context}"
        ...
        #     f"{context_str}"
        #     "## Jawaban Wajib:\n"
        #     "Gunakan pola: <jawaban>[n]. Contoh: Machine learning adalah...[1].\n"
        #     "Daftar referensi HARUS mengikuti format:\n"
        #     "**Referensi:**\n"
        #     "1. [Judul - Modul X Hal Y](link)\n"
        #     "2. [Judul - Modul A Hal B](link)\n"
        # )
